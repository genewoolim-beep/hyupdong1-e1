...
def parse_args():
    ...
    ap.add_argument('--force', action=argparse.BooleanOptionalAction, default=False,
                    help='그리기 시 Z축 힘제어. 기본 OFF(위치제어 — 검증됨). '
                         'ON(--force)은 펜던트에서 payload/TCP 캘리브레이션 후에만 쓰세요(안 하면 표면에 박힘)')
    ...
    while surface_z is None:
        ...
        if line == '':
            ...
            try:
                ...
                print("  ? 모르는 입력(Enter/숫자/u/done/q)")
...
def draw_svg_at_surface(args, surface_z: float, home):
    ...
    cfg = RobotConfig(
        ...
        draw_height_mm=surface_z,                 # ★ 측정한 표면 높이 그대로
    ...
    print(f"   Z 긋기 {surface_z:.1f} / 펜업 {surface_z + args.pen_up:.1f} mm")
    if args.force:
